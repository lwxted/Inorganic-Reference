...
#import "KSPListViewController.h"
#import "KSPListCell.h"
#import "Substance_With_Ksp.h"
@interface KSPListViewController ()

@end

@implementation KSPListViewController
...
{
    Substance_With_Ksp *element = [(KSPListCell *) sender.superview.superview current];
...
- (void)viewDidLoad
{
    ...
    self.title = @"Precipitation and Ksp";
    /*
     NSLog(@"Started");
    ...
    __weak KSPListViewController *weakSelf = self;
    ...
    dispatch_async(fetchQueue, ^{
        ...
        for (Substance_With_Ksp *reac in results) {
...
{
    static NSString *CellIdentifier = @"KSP Cell";
    KSPListCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (!cell) {
        cell = [[KSPListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"KSP Cell"];
    }
...
- (void) searchBarTextDidBeginEditing:(UISearchBar *)searchBar
{
    if (!self.pop) {
        if (!self.shownTip) {
            self.pop = [[CMPopTipView alloc] initWithMessage:NSLocalizedString(@"在这里输入沉淀化学式进行搜索。",nil)];
            self.shownTip = YES;
        ...
        dispatch_async(searchQueue, ^{
            ...
            if (!([str isEqualToString:@""] || [str isEqualToString:@","] || [str isEqualToString:@" "])) {
                if ([str rangeOfString:@","].location == NSNotFound && [str rangeOfString:@"，"].location == NSNotFound) {
                    ...
                    for (Substance_With_Ksp    *reac in arrayOfObjects) {
                        [arrayOfIDs addObject:[reac objectID]];
                    }
                    dispatch_async(dispatch_get_main_queue(), ^{
    ...
    dispatch_async(searchQueue, ^{
        ...
        if (!([str isEqualToString:@""] || [str isEqualToString:@","] || [str isEqualToString:@" "])) {
            if ([str rangeOfString:@","].location == NSNotFound && [str rangeOfString:@"，"].location == NSNotFound) {
                ...
                NSMutableArray *predicateArray = [[NSMutableArray alloc] initWithCapacity:arrayOfSearchStrings.count];
                for (NSString *str in arrayOfSearchStrings) {
                    [predicateArray addObject:[NSPredicate predicateWithFormat:@"(formula contains[c] %@)",search]];
                }
                NSPredicate *predicateCompound = [NSCompoundPredicate andPredicateWithSubpredicates:predicateArray];
                request.predicate = predicateCompound;
                request.entity = eD;
