//
//  ThermodynamicsListViewController.m
//  Inorganic Reference
//
...
#import "ThermodynamicsListViewController.h"
#import "Thermodynamics_Data.h"
#import "ThermodynamicsListCell.h"
#import "ThermodynamicsDetailViewController.h"

@interface ThermodynamicsListViewController ()

@end

@implementation ThermodynamicsListViewController
...
- (void)viewDidLoad
{
    ...
    self.HUD.labelText = NSLocalizedString(@"Loading Data...",nil);
    [self.HUD show:YES];
    [self.navigationController.view addSubview:self.HUD];
    __weak ThermodynamicsListViewController *weakSelf = self;
    ...
    dispatch_async(fetchQueue, ^{
        ...
        //NSSortDescriptor *sD = [[NSSortDescriptor alloc] initWithKey:@"tag" ascending:YES];
        NSPredicate *p = [NSPredicate predicateWithFormat:@"(tag != %@) AND (tag != %@)",@221,@222];
        request.predicate = p;
        request.entity = eD;
        //request.sortDescriptors = @[sD];
...
{
    Thermodynamics_Data *element = [(ThermodynamicsListCell *) sender.superview.superview current];
...
{
    static NSString *CellIdentifier = @"Thermodynamics List Cell";
    ThermodynamicsListCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (!cell) {
        cell = [[ThermodynamicsListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Thermodynamics List Cell"];
    }
...
- (void) searchBarTextDidBeginEditing:(UISearchBar *)searchBar
{
    if (!self.pop) {
        if (!self.shownTip) {
            self.pop = [[CMPopTipView alloc] initWithMessage:NSLocalizedString(@"在这里输入元素符号或物质进行搜索，可用逗号分开多个关键词。",nil)];
            self.shownTip = YES;
        ...
        dispatch_async(searchQueue, ^{
            ...
            if (!([str isEqualToString:@""] || [str isEqualToString:@","] || [str isEqualToString:@" "])) {
                if ([str rangeOfString:@","].location == NSNotFound && [str rangeOfString:@"，"].location == NSNotFound) {
                    ...
                    NSFetchRequest *request = [[NSFetchRequest alloc] init];
                    NSEntityDescription *eD = [NSEntityDescription entityForName:@"Thermodynamics_Data" inManagedObjectContext:context];
                    NSPredicate *p = [NSPredicate predicateWithFormat:@"(formula contains[c] %@) AND (tag != %@) AND (tag != %@)",search,@221,@222];
                    request.predicate = p;
                    request.entity = eD;
                    ...
                    for (Thermodynamics_Data    *reac in arrayOfObjects) {
                        [arrayOfIDs addObject:[reac objectID]];
                    }
                    dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    for (NSString *str in arrayOfSearchStrings) {
                        [predicateArray addObject:[NSPredicate predicateWithFormat:@"(formula contains[c] %@) AND (tag != %@) AND (tag != %@)",str,@221,@222]];
                    }
    ...
    dispatch_async(searchQueue, ^{
        ...
        if (!([str isEqualToString:@""] || [str isEqualToString:@","] || [str isEqualToString:@" "])) {
            if ([str rangeOfString:@","].location == NSNotFound && [str rangeOfString:@"，"].location == NSNotFound) {
                ...
                NSMutableArray *predicateArray = [[NSMutableArray alloc] initWithCapacity:arrayOfSearchStrings.count];
                for (NSString *str in arrayOfSearchStrings) {
                    [predicateArray addObject:[NSPredicate predicateWithFormat:@"(formula contains[c] %@) AND (tag != %@) AND (tag != %@)",search,@221,@222]];
                }
                NSPredicate *predicateCompound = [NSCompoundPredicate andPredicateWithSubpredicates:predicateArray];
                request.predicate = predicateCompound;
                request.entity = eD;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    detailViewController.thermodynamics = [self.displayItems objectAtIndex:indexPath.row];
    detailViewController.hidesBottomBarWhenPushed    = YES;
    [self.navigationController   pushViewController:detailViewController animated:YES];
    [self.tableView deselectRowAtIndexPath:indexPath animated:YES];
    detailViewController.title = @"Detailed Information";
}
